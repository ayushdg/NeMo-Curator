...
| `use_async_memory` | bool | True | Use CUDA asynchronous memory allocation for the shuffle. Set to `False` to use the legacy pool-wrapped CUDA memory resource. |
...
For recursive discovery behavior, custom suffix examples, and supported formats, see [Input File Discovery](/curate-text/process-data/deduplication#input-file-discovery).
...
removal_workflow = TextDuplicatesRemovalWorkflow(
    ...
    input_filetype="parquet",
    # Set input_file_extensions here too if identification used a custom suffix.
    id_field="_curator_dedup_id",
    duplicate_id_field="_curator_dedup_id",
...
- Set `duplicate_id_field` to match the ID column written by the identification workflow
