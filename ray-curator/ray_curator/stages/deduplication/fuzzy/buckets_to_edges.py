...
class BucketsToEdgesStage(ProcessingStage[FileGroupTask, FileGroupTask]):
    ...
    Args:
        ...
        read_kwargs: Keyword arguments to pass to the read_table function.
        write_kwargs: Keyword arguments to pass to the write_table function.
    """
    ...
    def process(self, task: FileGroupTask) -> FileGroupTask:
        ...
        output_fs = get_fs(self.output_dir, self.write_storage_options)
        output_dir = output_fs.sep.join([self.output_dir, self.name])
        output_fs.mkdirs(output_dir, exist_ok=True)
        output_path = output_fs.sep.join([output_dir, f"{task.task_id}.parquet"])
        pq.write_table(edges, output_path, filesystem=output_fs)
